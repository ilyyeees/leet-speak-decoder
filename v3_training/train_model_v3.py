...
The definitive training script for v3, designed for A100 GPU.

Key improvements over v2:
1. Reverse Pipeline: Clean -> LLM Corrupt -> Train (not Dirty -> LLM Clean)
2. Hybrid corruption: LLM handles semantics, script handles visuals
3. 10x multiplexing: 50k LLM calls -> 500k training pairs
4. Adversarial filtering: Keeps only hard examples v2 can't decode
5. Multi-phase training: Curriculum learning from easy to hard
6. Better base data: WikiText + ELI5 + LLM-generated content

Training Pipeline:
    1. Load 500k+ synthetic pairs from data_pipeline
    2. (Optional) Filter using existing v2 model
    3. Curriculum learning: easy -> medium -> hard intensity
    4. Progressive training with periodic validation
    5. A100-optimized hyperparameters with DeepSpeed/FSDP support

Hardware Requirements:
    - A100 40GB/80GB recommended
    - RTX 3090/4090 works with smaller batch sizes
...
Usage:
    ...
    # With adversarial filtering (requires v2 model)
    ...
    deepspeed --num_gpus=2 train_model_v3.py --data training_data_v3.jsonl --deepspeed
...
class TrainingConfig:
    ...
    Optimized for A100 80GB, adjust batch sizes for smaller GPUs.
    """

    # === Model Configuration ===
    base_model: str = "google/byt5-base"  # Start fresh for v3
    continue_from: Optional[str] = None  # Path to checkpoint to continue from
    use_v2_for_adversarial: str = "ilyyeees/byt5-leetspeak-decoder"  # v2 model for filtering
    ...
    # === Mixed Precision ===
    bf16: bool = True  # BF16 on A100 is faster and more stable than FP16
    fp16: bool = False
    tf32: bool = True  # Enable TF32 for faster matrix ops on Ampere+
...
def train(
    ...
    model_path = config.continue_from or config.base_model
...
Examples:
    # Basic training
    python train_model_v3.py --data data_pipeline/3_script_corruption/training_data_v3.jsonl
    ...
    # Model options
    parser.add_argument("--base-model", type=str, default="google/byt5-base",
                        help="Base model to fine-tune (default: google/byt5-base)")
    ...
    parser.add_argument("--adversarial", action="store_true",
                        help="Use v2 model to filter easy examples")
